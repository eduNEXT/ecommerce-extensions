###############################################
#
# Django eduNEXT e-commerce tenant commands.
#
###############################################
...
quality: clean ## check coding style with pycodestyle and pylint
	$(TOX) pycodestyle ./eco_tenant
	$(TOX) pylint ./eco_tenant --rcfile=./setup.cfg
	$(TOX) isort --check-only --recursive --diff ./eco_tenant

python-test: clean ## Run test suite.
	$(TOX) pip install -r requirements/test.txt --exists-action w
	$(TOX) coverage run --source ./eco_tenant manage.py test
